...
namespace Yqblog.Controllers
{
    ...
    public class HomeController : BaseController
    {
            ...
            switch (category.Type)
            {
                ...
                default:
                    ...
                    pager = category.SubCount > 0
                        ? MyService.GetArticlePaging(pager, 0, MyService.GetCategoryIds(id), iscommend, order)
        ...
        public ActionResult ArticleAjaxInfo(long id, long? orderId, string orderType)
        {
            ...
            {
                previousLink = "<a href=\"" + WebUtils.GetYqUrl(prearticle) + "\">" + prearticle.title + "</a>";
            }
            if (nextarticle != null && !string.IsNullOrWhiteSpace(nextarticle.title))
            {
                nextLink = "<a href=\"" + WebUtils.GetYqUrl(nextarticle) + "\">" + nextarticle.title + "</a>";
            }
        ...
        public ActionResult JsonArticleList(int cid, int? p)
        {
            ...
            if (cid > 0)
            {
                ...
                pager.PageSize = Configinfo.CatePagerCount;
                pager = category.SubCount > 0 ? MyService.GetArticlePaging(pager, 0, MyService.GetCategoryIds(cid)) : MyService.GetArticlePaging(pager, 0, cid);
            }
            else
            {
        ...
        public ActionResult AjaxGetArticleList(string data)
        {
            ...
            switch (paras.ArticleListType)
            {
                case "category":
                        ...
                        pager = category.SubCount > 0
                            ? MyService.GetArticlePaging(pager, 0, MyService.GetCategoryIds(paras.CategoryId), paras.Commend, paras.Order)
        ...
        private void SendCommentReplyEmail(blog_article comment)
        {
            ...
            var authorinfo = comment.username;
            if (comment.userid > 0)
            {
                authorinfo = "<a href=\"" + WebUtils.GetWebRootPath() + "/u/" + HttpUtility.UrlEncode(comment.username, System.Text.Encoding.UTF8) + "\" target=\"_blank\">" + comment.username + "</a>";
            }
        ...
        private void SendNoteReplyEmail(blog_article note)
        {
            ...
            var email = user.email.Trim();
            var authorinfo = note.username;
            if (note.userid > 0)
            {
                authorinfo = "<a href=\"" + WebUtils.GetWebRootPath()+"/u/" + HttpUtility.UrlEncode(note.username, System.Text.Encoding.UTF8) + "\" target=\"_blank\">" + note.username + "</a>";
            }
            var emailCulture = Configinfo.IfIndependentContentViaLang == 1 ? res.Lang : Configinfo.DefaultLang;
